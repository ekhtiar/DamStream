...
def write(dplid):
    ...
    for msg in balanced_consumer:
        ...
        for output in outputs:
            ...
            if output['tran_funcs'].lower().trim() != 'none':
                tranfuncs = dplmain.transfuncs.split(',')
                # Loging
                logging.info('transforming : '+tranfuncs+ 'without funcconfigs:'+funcconfigs)
                msg = transform(msg=msg, transfuncs = tranfuncs, funcconfigs=funcconfigs)
            ...
            if output['output_type'] == 'hdfs':
                ...
                logging.info('writing to '+directory + filename + str(msg.offset))
                hdfsclient.write(hdfs_path=directory + filename + str(msg.offset), data=msg.value)
                # Loging
